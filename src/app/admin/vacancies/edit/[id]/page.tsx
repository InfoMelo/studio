...
import { updateVacancy, getVacancy } from '@/app/admin/actions';
...
export default function EditVacancyPage() {
        ...
        if (docId) {
            getVacancy(docId).then(vacancy => {
                if (vacancy) {
                    ...
                    toast({ variant: "destructive", title: "Gagal", description: "Data lowongan tidak ditemukan." });
    ...
    async function onSubmit(data: VacancyFormValues) {
        try {
            await updateVacancy(docId, {...data, deadline: data.deadline.toISOString()});
            toast({ title: "Sukses", description: "Lowongan berhasil diperbarui." });
            router.push('/admin/vacancies');
        } catch (error) {
            toast({ variant: "destructive", title: "Gagal", description: "Terjadi kesalahan saat memperbarui lowongan." });
        }
    }
