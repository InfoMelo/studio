...
export default function PartnerActions({ partner }: PartnerActionsProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      if (partner.docId) {
        await deletePartner(partner.docId);
        toast({ title: "Sukses", description: "Data mitra berhasil dihapus." });
      }
    } catch (error) {
  ...
  return (
    <>
      <Button variant="ghost" size="icon" asChild>
        <Link href={`/admin/partners/edit/${partner.docId}`}>
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini tidak bisa dibatalkan. Data mitra <strong>{partner.name}</strong> akan dihapus secara permanen.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
